...
class ConformalModel(nn.Module):
    def __init__(
        ...
        self.model = model
        if isinstance(calib_loader.dataset, torch.utils.data.Subset):
        ...
        if T == None:
            T = 1.3
        self.T = torch.Tensor([T])  # initialize (1.3 is usually a good value)
        self.T, self.calib_logits = platt(self, calib_loader, device, dataset)
        self.randomized = randomized
        self.allow_zero_sets = allow_zero_sets

        if kreg is None or lamda is None:
            kreg, lamda, self.calib_logits = pick_parameters(
                model,
                self.calib_logits,
                alpha,
                kreg,
        ...
        self.calib_loader = tdata.DataLoader(
            self.calib_logits, batch_size=batch_size, shuffle=False, pin_memory=True
        )

        self.Qhat = conformal_calibration_logits(self, self.calib_loader)
...
# Temperature scaling
def platt(
    cmodel, calib_loader, device, dataset=None, max_iters=10, lr=0.01, epsilon=0.01
):
    print("Begin Platt scaling.")
    # Save logits so don't need to double compute them
    logits_dataset = get_logits_targets(
        cmodel.model, calib_loader, device, cmodel.num_classes, dataset=dataset
    )
    ...
    # T = platt_logits(cmodel, logits_loader, max_iters=max_iters, lr=lr, epsilon=epsilon, device=device)
    # return the temperature in the config file, as the temperature tuning is not working well.
    T = cmodel.T
    ...
    def __init__(
        ...
        if not naive and not LAC:
            ...
            scores_cal = 1 - np.array(
                [
                    np.sort(
                        torch.softmax(calib_loader.dataset[i][0] / self.T.item(), dim=0)
                    )[::-1][gt_locs_cal[i]]
...
def platt_logits(
    cmodel, calib_loader, max_iters=10, lr=0.01, epsilon=0.01, device=None
):
    nll_criterion = nn.CrossEntropyLoss().to(device)

    T = nn.Parameter(torch.Tensor([cmodel.T]).to(device))
...
def pick_lamda_size(
    model, paramtune_loader, alpha, kreg, randomized, allow_zero_sets, device, dataset
):
...
def pick_parameters(
    ...
    print(f"Using kreg: {kreg}, and lamda: {lamda}, chosen automatically.")
